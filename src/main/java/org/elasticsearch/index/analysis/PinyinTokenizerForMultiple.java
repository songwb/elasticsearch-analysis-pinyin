...
public class PinyinTokenizerForMultiple extends Tokenizer {
    ...
    StringBuilder firstLetterPrefix;
//    SoftReference<LinkedHashSet<String>> firstLettersSet;
    LinkedHashSet<String> firstLettersSet;
    ...
    public PinyinTokenizerForMultiple(PinyinConfig config) {
        ...
        firstLettersSet = new LinkedHashSet<>();
        fullPinyinLetters = new StringBuilder();
    }

    public PinyinTokenizerForMultiple(int bufferSize) {
        super();
    ...
    public final boolean incrementToken() throws IOException {
        ...
        if (!done) {
            ...
            if (!processedCandidate) {
                ...
                List<String> pinyinList = Pinyin.pinyinForMultiple(source);
                ...
                for (int i = 0; i < source.length(); i++) {
                    Set<String> donePinyinSet= Sets.newHashSet();
                    char c = source.charAt(i);
                    List<String> pinyinListForSingleChar = Pinyin.pinyinForMultiple(c+"");
                    ...
                    if (c < 128) {
                        ...
                        boolean doPosition = false;
                        for(int w = 0 ; w < pinyinListForSingleChar.size();w++){
                            String pinyin = pinyinListForSingleChar.get(w);
                            if (StringUtil.isNotBlank(pinyin)) {
                                if(donePinyinSet.contains(pinyin)){
                                    continue;
                                }
                                if(!doPosition){
                                    position++;
                                ...
                                if(firstLettersSet != null && firstLettersSet.size()>0){
                                    LinkedHashSet<String> firstLettersSetTmp = new LinkedHashSet<>();
                                    Iterator<String> firstLetterIterator = firstLettersSet.iterator();
                                    while (firstLetterIterator.hasNext()){
                                        StringBuilder firstLetterBuilder = new StringBuilder().append(StringUtils.uncompress(firstLetterIterator.next()));
                                        if(w == 0){
                                            firstLetterBuilder.append(pinyin.charAt(0));
                                        }else{
                                            firstLettersSetTmp.add(StringUtils.compress(firstLetterBuilder.toString()));
                                            StringBuilder stringBuilder = new StringBuilder();
                                            for(int k = 0; k<firstLetterBuilder.length()-1; k++){
                                                stringBuilder.append(firstLetterBuilder.charAt(k));
                                            }
                                            stringBuilder.append(pinyin.charAt(0));
                                            firstLetterBuilder = stringBuilder;
//                                            firstLetterBuilder = new StringBuilder(firstLetterBuilder.substring(0,firstLetterBuilder.length()-1)+pinyin.charAt(0));
                                        }
                                        firstLettersSetTmp.add(StringUtils.compress(firstLetterBuilder.toString()));
                                    }
                                    firstLettersSet = firstLettersSetTmp;
                                }else {
                                    firstLetters.append(pinyin.charAt(0));
                                    firstLettersSet.add(StringUtils.compress(firstLetters.toString()));
                                }
...
//            }
            ...
            if (config.keepFirstLetter && firstLettersSet.size() > 0 && !processedFirstLetter) {
                processedFirstLetter = true;
                for(String flTmp : firstLettersSet){
                    String firstLetterTmp = StringUtils.uncompress(flTmp);
                    String fl;
                    if (firstLetterTmp.length() > config.LimitFirstLetterLength && config.LimitFirstLetterLength > 0) {
//                        fl = firstLetterTmp.substring(0, config.LimitFirstLetterLength);
                        fl = firstLetterTmp;
                    } else {
                        fl = firstLetterTmp;
                    }
    ...
    public static void main(String[] args){
        ...
        firstLetterBuilder.append("qaz");
        Set<String> pinyinSet = Sets.newHashSet();
        for(int i = 0 ; i < 1000;i++){
            pinyinSet.add("qazwsxedcrfvtgbyhnujmikolpqazwsxedcrfvtgbyhnujmikolpqazwsxedcrfvtgbyhnujmikolp"+i);
        }
        Iterator<String> pinyinIterator = pinyinSet.iterator();
        while (pinyinIterator.hasNext()){
            String pinyin = pinyinIterator.next();
            firstLetterBuilder = new StringBuilder(firstLetterBuilder.substring(0,firstLetterBuilder.length()-1)+pinyin.charAt(0));
        }
    }
